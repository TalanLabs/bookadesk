...
describe('DeleteBooking', () => {
    ...
    let mockEmailGateway: EmailGateway = {
    ...
    test('should not send an email if user deletes its own booking', async () => {
        ...
        // WHEN
        await deleteBooking(booking.id, userEmail, isAdmin, mockBookingRepo, mockEmailGateway)
    ...
    test('should send an email to booking\'s user when admin deletes its booking', async () => {
        // GIVEN
        let isAdmin = true
        let userEmail = "admin-user@mail.com"
    ...
    test('should throw error if deleting user is nor admin nor booking\'s user', async () => {
        // GIVEN
        let isAdmin = false
        let userEmail = "not-admin-user@mail.com"

        // WHEN
        await expect(deleteBooking(booking.id, userEmail, isAdmin, mockBookingRepo, mockEmailGateway))
            .rejects
        // THEN
