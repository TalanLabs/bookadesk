        ...
        <div class="list-offices_container view">
            <div
                ...
                <div v-if="office.floors.length > 0">
                    ...
                    <div v-for="floor in office.floors"
                         :key="floor.id"
                         class="list-offices_floors row">
                        <div class="cell">{{floor.name}}</div>
                        <div class="cell-action">
                            <button class="edit-button icon-button clickable"
                                    v-show="!editMode"
                                    @click="$router.push(`/edit-places/${office.id}/${floor.id}`)"></button>
                        </div>
                    </div>
                </div>
                <div v-if="editMode">
                    ...
                    <div class="add-floor-form">
                        <label>
                            <input
                                    ...
                                    v-on:keyup.enter="() => floorNames[office.id] && addFloor(office.id)"
                                    required
                            />
                        </label>
                        <button @click="addFloor(office.id)" :disabled="!floorNames[office.id]">Ajouter</button>
                    </div>
                </div>
            </div>
        <div v-if="editMode">
            ...
            <div class="add-office-form">
                ...
                <button @click="addOffice" :disabled="!isOfficeNameValid">Ajouter</button>
            </div>
        </div>
        <button
                @click="save"
